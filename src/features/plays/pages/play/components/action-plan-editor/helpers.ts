// Parse plain text content and convert variables to editor format
export function parseContent(text: string): any {
  ...
  const parseParagraph = (paragraphText: string) => {
    // Regular expression to match ${type:value:label}$ format
    ...
    // Return content array, or empty array if no content
    return content
  }
  ...
  const contentParagraphs = paragraphs
    .map((p) => {
      ...
      // Only create paragraph if it has content or if it's an intentionally empty paragraph
